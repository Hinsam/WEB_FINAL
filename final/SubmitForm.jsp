...
<%
	...
	if (isPost) {
		...
		try {
			...
			String insertQuery = "INSERT INTO trade_requests (UserId, SaleId, Comment, RequestDate, RentalStartDate, RentalEndDate, approval) VALUES (?, ?, ?, ?, ?, ?, ?)";
			...
			insertPstmt.setString(7, approval);
...
<%
	...
	String checkOwnershipQuery = "SELECT * FROM trade_requests WHERE SaleId=? AND UserId=?";
    ...
    checkOwnershipPsmt.setString(2, userId);
    ResultSet ownershipResult = checkOwnershipPsmt.executeQuery();
    
    if (ownershipResult.next()) {
    	out.println("<script>alert('신청은 한 번만 가능합니다.'); window.location='post_list.jsp';</script>");
    }
%>
